...
def plot_similarity_matrix(matrix, labels_a=None, labels_b=None, ax: plt.Axes=None, title=""):
    ...
    img = ax.matshow(matrix, extent=(-0.5, matrix.shape[0] - 0.5, -0.5, matrix.shape[1] - 0.5))
...
def interactive_diarization(similarity_dict, wav, wav_splits, x_crop=5, show_time=False):
    ...
    def update(i):
        ...
        if similarity > 0.50:
            message = "Speaker: %s" % name
            color = _default_colors[best]
            if name=='Interviewee':
                print("I  "+str(times[i]))
                time_speaker.append(times[i])
    ...
    print(time_speaker)
    print("Speech start time 1st question " + str(time_speaker[0]))
    print("Speech end time last question " + str(time_speaker[-1]))


    for i, j in enumerate(time_speaker[:-1]):
        if (time_speaker[i + 1]-j)>=1:
            print(time_speaker[i + 1])
            print(j)
